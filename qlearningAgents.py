...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        maxQNext = self.computeValueFromQValues(nextState)
        transBias = reward + self.discount * maxQNext - self.Q[(state, action)]
        self.Q[(state, action)] += self.alpha * transBias
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feat in features:
            featVal += self.weights[feat] * features[feat]
    ...
    def update(self, state, action, nextState, reward):
        ...
        for nextAct in nextActions:
            maxQNext = max(maxQNext, self.getQValue(nextState, nextAct))

        if len(nextActions) == 0:
            maxQNext = 0

        different = reward + (self.discount * maxQNext) - self.getQValue(state, action)
        newWeight = self.weights.copy()
        features = self.featExtractor.getFeatures(state, action)
        for feat in features:
            newWeight[feat] += self.alpha * different * features[feat]
